...
//Automation Script to click on "Publish" products.

public class ProductCatalogTest_11 extends BaseClassTest {
	...
	@Test(priority = 3)
	public void clickOnPublishedFromDropdown() {


		logger = extentReports.createTest("Click on Published");

		ItemCatalog catalogBulkUploadAction = PageFactory.initElements(driver,
				ItemCatalog.class);

		logger.info("Published option from dropdown");
		
		
		try {
			catalogBulkUploadAction.selectDropdownWithPublished();
		} catch (InterruptedException e) {
			
			e.printStackTrace();
		}
		
		logger.pass("Published option Clicked Successfully");
