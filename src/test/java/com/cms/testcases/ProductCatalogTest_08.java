...
//Automation Script to verify "eye" icon"

public class ProductCatalogTest_08 extends BaseClassTest {
	...
	public void verifyEyeIcon() {
		...
		logger = extentReports.createTest("Verify eye icon & number of count");

		ItemCatalog catalogBulkUploadAction = PageFactory.initElements(driver,
				ItemCatalog.class);
		
		logger.info("Eye icon & number Verified Successfully");
		
		try {
			System.out.println("The eye icon is display ----- " + catalogBulkUploadAction.eyeIcon());
			
			System.out.println("The eye icon Count is display ----- " + catalogBulkUploadAction.eyeIconCount());
		...
		logger.pass("Eye icon & number Verified ");
