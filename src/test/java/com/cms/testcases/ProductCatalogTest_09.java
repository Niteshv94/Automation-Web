...
// Automation Script to verify "download" icon

public class ProductCatalogTest_09 extends BaseClassTest {
	...
	public void verifyEyeIcon() {
		...
		logger = extentReports.createTest("Verify download icon & number of count");

		ItemCatalog catalogBulkUploadAction = PageFactory.initElements(driver,
				ItemCatalog.class);
		
		logger.info("Download icon & number Verified Successfully");
		
		try {
			System.out.println("The download icon is display ----- " + catalogBulkUploadAction.downloadIcon());
			
			System.out.println("The download icon Count is display ----- " + catalogBulkUploadAction.downloadIconCount());
		...
		logger.pass("Download & number Verified ");
