 ...
 *
 */
//Automation Script to verify "Unpublished" Products

public class ProductCatalogTest_12 extends BaseClassTest {
	...
	@Test(priority = 3)
	public void clickOnUnpublishedFromDropdown() {


		logger = extentReports.createTest("Click on Unpublished");

		ItemCatalog catalogBulkUploadAction = PageFactory.initElements(driver,
				ItemCatalog.class);

		logger.info("Unpublished option from dropdown");
		
		
		try {
			catalogBulkUploadAction.selectDropdownWithUnPublished();
		} catch (InterruptedException e) {
			
			e.printStackTrace();
		}
		
		logger.pass("Unpublished option Clicked Successfully");
